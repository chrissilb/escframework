...
public abstract class State<T> {
	...
	static {
        eventProcessor = new PNBuilder<Event>("transition")
        	.add(new Scheduler<>())
...
//    	if (this.getClass().getSimpleName().equals("State")) {
//    		name = this.getClass().getSimpleName() + " " + name;
//    	}
//    	else {
//    		name = this.getClass().getSimpleName().substring(0, this.getClass().getSimpleName().length() - 5) + " " + name;
//    	}
    	
        this.name = name;
        
        transitionMode = new DirectTransition<T>();
    ...
    protected void setStateValue(T value) {
        ...
        while (stateValue != value) {
            ...
            if (activityHandler != null && !activityHandler.activity(newValue, stateValue)) return;
