...
/**
 * A {@code TimerHandler} redirects events to the timer processor network if
 * their push time is in the future (see {@link TimerFactory}). Corresponding
 * {@link Callback}s are collected and forwarded to the actual processor
 * network.
 * 
...
//			assert SwingUtilities.isEventDispatchThread();
	...
	/**
	 * Constructs a {@code TimerHandler}.
	 * 
	 * @param name  the name of this {@code Processor}
	 * @param timer the entry point of the timer processor network to delay events
	 * @see TimerFactory
	 */
