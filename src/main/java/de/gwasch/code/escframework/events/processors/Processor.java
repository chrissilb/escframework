...
/**
 * {@code Processor} is the main class of all processors. It provides:
 * <ul>
 * <li>Process events by the current processor</li>
 * <li>Forward events to the next processor(s)</li>
 * <li>Handle callback events</li>
 * </ul>
 * It distinguishes between the following event types:
 * <ul>
 * <li>Process events which are the "actual" events processed by the processor
 * network</li>
 * <li>Activate events</li>
 * <li>Deactivate events</li>
 * <li>Suspend events</li>
 * <li>Resume events</li>
 * <li>Cancel events</li>
 * </ul>
 * This structure follows the {@link Event} interface.
 * 
 * @param <E> the event type considered by the {@code Processor}
 */
public abstract class Processor<E extends Event> implements Iterable<Processor<E>> {
	...
	public final <F extends Event> ProcessListener<Event> getListener(Class<F> eventClass) {
		ProcessListener<Event> listener = (ProcessListener<Event>) handlerMap.get(eventClass);
		return listener;
	}

	// todo, uninstall geht noch nicht
	public final <F extends Event> void installListener(Class<F> eventClass,
			ProcessListener<? super F> processListener) {

		handlerMap.put(eventClass, (ProcessListener<Event>) processListener);
		...
		if (predecessor.getSuccessorsCount(eventClass) == 0) { // add
			for (Processor<E> predecessor : this) {
				...
				predecessor.addSuccessorHandler(eventClass, processListener);
			}
		} else { // update
			...
			predecessor.successorsMap.get(eventClass).set(index, (ProcessListener<Event>) processListener);
	...
	public final <F extends Event> void processGeneric(Class<F> eventClass, F event,
			CallbackListener<? super F> callbackListener) {
	...
	public final void forwardActivate(int successorIndex, ActivateEvent event,
			CallbackListener<ActivateEvent> callbackListener) {
	...
	public final void forwardDeactivate(int successorIndex, DeactivateEvent event,
			CallbackListener<DeactivateEvent> callbackListener) {
		forwardGeneric(successorIndex, event, callbackListener);
	}
	...
	public final void forwardSuspend(int successorIndex, SuspendEvent event,
			CallbackListener<SuspendEvent> callbackListener) {
		forwardGeneric(successorIndex, event, callbackListener);
	}
	...
	public final void forwardResume(int successorIndex, ResumeEvent event,
			CallbackListener<ResumeEvent> callbackListener) {
		forwardGeneric(successorIndex, event, callbackListener);
	}
	...
	public final void forwardCancel(int successorIndex, CancelEvent event,
			CallbackListener<CancelEvent> callbackListener) {
		forwardGeneric(successorIndex, event, callbackListener);
	}
